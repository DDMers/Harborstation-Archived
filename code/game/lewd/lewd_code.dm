...
/mob/proc/cum(mob/partner, target_orifice)
		...
		switch(target_orifice)
			if(CUM_TARGET_MOUTH)
				if(partner.has_mouth() && partner.mouth_is_free())
					...
					partner.reagents.add_reagent("cum", pick(8,9,10,11))
				else
					message = "cums on \the [partner]'s face."
					partner.reagents.add_reagent("cum", pick(2,3,4,5,6,7,8))
			if(CUM_TARGET_THROAT)
				if(partner.has_mouth() && partner.mouth_is_free())
					message = "shoves deep into \the [partner]'s throat and cums."
					partner.reagents.add_reagent("cum", pick(10,11,12,13,14,15,16))
				else
					message = "cums on \the [partner]'s face."
					partner.reagents.add_reagent("cum", pick(2,3,4,5,6,7,8))
			if(CUM_TARGET_VAGINA)
				if(partner.is_nude() && partner.has_vagina())
					...
					partner.reagents.add_reagent("cum", pick(8,9,10,11,12,13,14))
				else
	...
	if(multiorgasms > (sexual_potency*0.34)) //accurate enough, i dont want to get -0 in these
		refactory_period = 40 //sex cooldown
		src.set_drugginess(pick(30,35,40))
	else
		refactory_period = 40
...
/mob/proc/do_normalfuck(mob/partner) //does not require special admin perms
	var/mob/living/carbon/human/H = src
...
/mob/proc/do_oral(mob/partner)
	var/message
	var/lust_increase = pick(8,9,10,11,12,13,14)
...
/mob/proc/do_facefuck(mob/partner)
	var/message
	var/lust_increase = pick(9,10,11,12,13,14)
...
/mob/proc/do_throatfuck(mob/partner)
	var/message
	var/lust_increase = pick(9,10,11,12,13,14,15,16)
	...
	if(more_lewd_erp == TRUE)
		if(is_fucking(partner, CUM_TARGET_THROAT))
			...
			if(rand(3) && istype(partner, /mob/living/carbon/human))
...
/mob/proc/do_anal(mob/partner)
	var/message
	var/lust_increase = pick(9,10,11,12,13,14,15)
...
/mob/proc/do_mountass(mob/partner)
	var/message
	var/lust_increase = pick(9,10,11,12,13)
...
/mob/proc/do_handjob(mob/partner)
	var/message
	var/lust_increase = pick(8,9,10,11,12,13)
